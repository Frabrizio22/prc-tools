...
# This script generates all 44 product pages for PRC Peptides
...
create_page() {
    ...
    local desc="$7"
    local full_desc="$8"
    local applications="$9"
    local storage="${10}"
    local related="${11}"

    cat > "$PRODUCTS_DIR/$slug.html" <<'HTMLEOF'
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>PRODUCT_NAME | CAT_NAME | PRC Peptides</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .product-category { display: inline-block; font-size: 0.65rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: CAT_COLOR; margin-bottom: 8px; }
...
<body>
    ...
    <div class="breadcrumb">
        <a href="../shop.html">Home</a> › <a href="../shop.html#categories">CAT_NAME</a> › PRODUCT_NAME
    </div>

    <section class="product-section">
        <div class="product-header">
            <div class="product-category">CAT_NAME</div>
        ...
        <div class="product-desc">SHORT_DESC</div>
        ...
        <div class="action-buttons">
            <button class="btn-primary" onclick="addToCart('PRODUCT_NAME_JS', PRICE)">Add to Cart — $PRICE</button>
            <a href="mailto:support@prcpeptides.com?subject=COA Request: PRODUCT_NAME" class="btn-secondary">Request COA</a>
        </div>

        <div class="related-products">
            <h2>Related Products</h2>
            <div class="related-grid">RELATED_PRODUCTS
            </div>
        </div>
    </section>
...
HTMLEOF
    ...
    sed -i '' "s/PRODUCT_NAME/$name/g" "$PRODUCTS_DIR/$slug.html"
    sed -i '' "s/CAT_NAME/$cat_name/g" "$PRODUCTS_DIR/$slug.html"
    sed -i '' "s/CAT_COLOR/$cat_color/g" "$PRODUCTS_DIR/$slug.html"
    sed -i '' "s/PRICE/$price/g" "$PRODUCTS_DIR/$slug.html"
    sed -i '' "s/SHORT_DESC/$desc/g" "$PRODUCTS_DIR/$slug.html"
    sed -i '' "s/FULL_DESC/$full_desc/g" "$PRODUCTS_DIR/$slug.html"
    sed -i '' "s/STORAGE/$storage/g" "$PRODUCTS_DIR/$slug.html"
    sed -i '' "s/PRODUCT_NAME_JS/${name//\'/\\\'}/g" "$PRODUCTS_DIR/$slug.html"
    
    # Handle applications section
    if [ -n "$applications" ]; then
        echo "        <h2>Research Applications</h2>
        <ul>
$applications
        </ul>" > /tmp/apps_section.txt
        # This is complex in bash, skipping for now
    fi
    
    echo "Created: $slug.html"
}

echo "Building all 44 product pages..."
echo "This script template is ready but needs full product data"
echo "Use Node.js script instead for better templating"
